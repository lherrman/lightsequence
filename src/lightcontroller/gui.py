...
class LaunchpadButton(QPushButton):
    ...
    def set_color(self, color: LaunchpadColor):
        ...
        if color == LaunchpadColor.OFF:
            bg_color = "#2a2a2a"
        elif color in [LaunchpadColor.RED_LOW, LaunchpadColor.RED_MID, LaunchpadColor.RED_FULL]:
            intensity = ["#4a0000", "#8a0000", "#ff0000"][color - 1] if color <= 3 else "#ff0000"
            bg_color = intensity
        elif color in [LaunchpadColor.GREEN_LOW, LaunchpadColor.GREEN_MID, LaunchpadColor.GREEN_FULL]:
            bg_color = "#00ff00"
        elif color in [LaunchpadColor.AMBER_LOW, LaunchpadColor.AMBER_MID, LaunchpadColor.AMBER_FULL]:
            bg_color = "#ffaa00"
        elif color in [LaunchpadColor.YELLOW_LOW, LaunchpadColor.YELLOW_MID, LaunchpadColor.YELLOW_FULL]:
            bg_color = "#ffff00"
        else:
            bg_color = "#404040"
...
class ControlPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.scene_list.setMaximumHeight(150)
        
        scene_buttons = QHBoxLayout()
        self.activate_scene_btn = QPushButton("Activate Scene")
        self.deactivate_scene_btn = QPushButton("Deactivate Scene")
        self.blackout_btn = QPushButton("Blackout")
        
        scene_buttons.addWidget(self.activate_scene_btn)
        scene_buttons.addWidget(self.deactivate_scene_btn)
        scene_buttons.addWidget(self.blackout_btn)
        
        scene_layout.addWidget(self.scene_list)
        scene_layout.addLayout(scene_buttons)
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.launchpad_viz.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
    ...
    def _setup_connections(self):
        ...
        self.control_panel.activate_scene_btn.clicked.connect(self._activate_selected_scene)
        self.control_panel.deactivate_scene_btn.clicked.connect(self._deactivate_selected_scene)
        self.control_panel.blackout_btn.clicked.connect(self._blackout)
        
        self.control_panel.activate_preset_btn.clicked.connect(self._activate_selected_preset)
        ...
        self.control_panel.rainbow_btn.clicked.connect(lambda: self._play_effect("rainbow"))
        self.control_panel.chase_btn.clicked.connect(lambda: self._play_effect("chase"))

    def _update_lists(self):
        """Update scene and preset lists."""
        # Update scenes
        self.control_panel.scene_list.clear()
        for scene_name in self.config.scenes.keys():
            self.control_panel.scene_list.addItem(scene_name)
    ...
    def _activate_selected_scene(self):
        """Activate selected scene."""
        current_item = self.control_panel.scene_list.currentItem()
        if current_item and self.controller:
            scene_name = current_item.text()
            self.controller._toggle_scene(scene_name)
            self.status_panel.append_status(f"Toggled scene: {scene_name}")

    def _deactivate_selected_scene(self):
        """Deactivate selected scene."""
        current_item = self.control_panel.scene_list.currentItem()
        if current_item and self.controller:
            scene_name = current_item.text()
            self.controller._deactivate_scene(scene_name)
            self.status_panel.append_status(f"Deactivated scene: {scene_name}")
    ...
    def _update_status(self):
        ...
        if active_preset_idx is not None:
            coords = self.controller.launchpad.preset_coords_from_index(active_preset_idx)
            if coords:
                x, y = coords
