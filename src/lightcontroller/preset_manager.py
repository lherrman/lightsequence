...
class PresetManager:
    ...
    def __init__(self, feedback_callback: Optional[Callable[[int, int, bool], None]] = None):
    ...
    def _create_default_presets(self):
        ...
        default_presets = [
            ...
            {"name": "Back Row", "desc": "Last row only", "scenes": list(range(32, 40))},
            {"name": "Alternating", "desc": "Every other scene", "scenes": [i for i in range(40) if i % 2 == 0]},
            {"name": "Center", "desc": "Center scenes", "scenes": [18, 19, 20, 21, 26, 27, 28, 29]},
            {"name": "Corners", "desc": "Corner scenes", "scenes": [0, 7, 32, 39]},
            {"name": "Cross", "desc": "Cross pattern", "scenes": [3, 4, 11, 12, 19, 20, 27, 28, 35, 36]},
            {"name": "Blackout", "desc": "All off", "scenes": []},
            
            # Row 1 (y=6 in grid coordinates)  
            {"name": "Rows 1&3", "desc": "First and third rows", "scenes": list(range(8)) + list(range(16, 24))},
            {"name": "Rows 2&4", "desc": "Second and fourth rows", "scenes": list(range(8, 16)) + list(range(24, 32))},
            {"name": "Left Half", "desc": "Left columns", "scenes": [i for i in range(40) if i % 8 < 4]},
            {"name": "Right Half", "desc": "Right columns", "scenes": [i for i in range(40) if i % 8 >= 4]},
            {"name": "Checkerboard", "desc": "Checkerboard pattern", "scenes": [i for i in range(40) if (i % 8 + i // 8) % 2 == 0]},
            {"name": "Border", "desc": "Border only", "scenes": [0,1,2,3,4,5,6,7,8,15,16,23,24,31,32,33,34,35,36,37,38,39]},
            {"name": "Inside", "desc": "Inside only", "scenes": [9,10,11,12,13,14,17,18,19,20,21,22,25,26,27,28,29,30]},
    ...
    def update_preset_scenes(self, preset_index: int, scene_indices: List[int]):
        ...
        if preset_index in self.presets:
            ...
            logger.info(f"Updated preset {preset_index} with {len(scene_indices)} scenes")
